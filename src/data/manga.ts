...
const MANGA_DATA = {
  ...
  chapters: Array.from({ length: 100 }, (_, i) => ({
    ...
    pages: Array.from({ length: 15 }, (_, j) => 
      `https://picsum.photos/seed/manga-${i + 1}-${j + 1}/800/1200`
    )
  }))
};
...
export const preloadThumbnails = (startIndex: number, count: number) => {
  MANGA_DATA.chapters.slice(startIndex, startIndex + count).forEach(chapter => {
...
export const getChapter = (id: string) => MANGA_DATA.chapters.find(chapter => chapter.id === id);